...
$conn = new Conexion();
$sql = "SELECT nombre, apellidos, telefono, email, bankaccount FROM usuario
...
if(!$res) {
    ...
    if($bank != ''){ //Si no es vacío
        $bank = decryptBankNumber($bank);
    }
}
...
<form action="modify_account.php" method="post" id="preferences_form">
    ...
    <input type="text" id="pref_bank" name="bank" class="form-control form-control-sm" maxlength="20" value="<?=$bank?>">
